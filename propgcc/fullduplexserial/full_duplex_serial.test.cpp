...
void test_BasicRxTx(void) {
    char letter = 'A';
    sut->Put(letter);
    TEST_ASSERT_EQUAL_INT(letter, sut->Get());
}

void test_RxcheckWithNothingInBuffer(void) {
    TEST_ASSERT_EQUAL_INT(-1, sut->Get(TIMEOUT));
}

void test_BasicRxCheckTx(void) {
    char letter = 'C';
    sut->Put('C');
...
void test_PutGetMultipleBytes(void) {
...
void test_RxflushClearsBuffer(void) {
    char letter1 = 'E';
    char letter2 = 'F';
    sut->Put(letter1);
    waitcnt(CLKFREQ / 100 + CNT);
