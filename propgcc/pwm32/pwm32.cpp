...
//extern char _load_start_pwm32_cog[];
...
void PWM32::Start(void) {
    ...
    */
    ...
    cog = cognew((int) (&(*(int *) &dat[0])), (int) (&command)) + 1;
}

void PWM32::Duty(int pin, int duty_cycle, int period) {
    if (period != 0) {
        if (duty_cycle == 0) {
            Dutymode(pin, 2);
            Statemode(pin, 1);
...
void PWM32::Servo(int Pin, int Pulsewidth) {
    int Ton, Toff;
    if (Pulsewidth != 0) {
        Ton = ((Pulsewidth * 1000) / Resolution);
...
void PWM32::PWM(int Pin, int Ontime, int Offtime) {
    InternalPWM(Pin, (Ontime * 1000) / Resolution, (Offtime * 1000) / Resolution);

}

void PWM32::InternalPWM(int Pin, int Ontime, int Offtime) {
...
void PWM32::Statemode(int Pin, int State) {
    argument_0 = Pin;
    argument_1 = State;
    command = Io_state;
    while (!(command == 0)) {
...
void PWM32::Dutymode(int Pin, int Mode) {
    argument_0 = Pin;
    argument_1 = Mode;
...
void PWM32::Phasesync(int Pin1, int Pin2, int Phase) {
